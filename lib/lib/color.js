...
exports.getPageColors = getPageColors;
exports.toHexRgb = toHexRgb;

/* eslint-disable no-param-reassign */

/** Convert a rgb(...) or rgba(...) string to its hexadecimal representation. */
function toHexRgb(rgbColor) {
  if (!rgbColor || !rgbColor.startsWith('rgb')) {
    return rgbColor;
  } // eslint-disable-next-line prefer-template


  return "#" + rgbColor.replace(/rgba?\((.+)\)/, '$1').split(',').slice(0, 3).map(function (x) {
    return x.trim();
  }).map(function (x) {
...
function parseRgb(rgbColor) {
  return rgbColor.replace(/rgba?\((.+)\)/, '$1').split(',').map(function (x) {
...
function luminance(_ref) {
  var r = _ref[0],
      g = _ref[1],
      b = _ref[2];
  var colors = [r, g, b].map(function (v) {
    var vSrgb = v / 255;
...
function contrast(colorA, colorB) {
  ...
  var luminanceA = luminance(colorA);
  var luminanceB = luminance(colorB);
...
function getPageColors(imgData) {
  var colors = {}; // Data is a flat array containing the image pixels as uint8 RGBA values in the range [0, 255]
  ...
  for (var i = 0; i < imgData.length - 3; i += 4) {
    ...
    var b = imgData[i + 2];
    var rgb = "rgb(" + r + "," + g + "," + b + ")";
    colors[rgb] = ((_colors$rgb = colors[rgb]) !== null && _colors$rgb !== void 0 ? _colors$rgb : 0) + 1;
  ...
  var sorted = Object.entries(colors).sort(function (_ref2, _ref3) {
    var freqA = _ref2[1];
    var freqB = _ref3[1];
    return freqB - freqA;
  });
  var textColor = sorted[0][0]; // Add fallback colors to list of candidate colors

  sorted.push(['rgba(0, 0, 0)', 0], ['rgb(255, 255, 255)', 0]);
  var bgColor = sorted.slice(1).find(function (_ref4) {
    var color = _ref4[0];
    return contrast(textColor, color) >= 7;
  })[0];
  return {
    textColor: textColor,
    bgColor: bgColor
